...
- `# check`는 최소 `## logic_checklist`, `## ui_checklist` 두 하위 섹션으로 유지한다. 둘을 하나의 단일 bullet 목록으로 합치면 실패다.
...
- 유닛테스트 통과만으로 완료 처리하지 않는다. `job.md#check evidence` 실행 근거가 있어야 한다.
...
- JS/TS 계열:
  ...
  - ad-hoc Playwright node script를 작성할 때는 정규식/selector escape 실수로 스크립트가 깨지지 않도록 `ORC_QA_PLAYWRIGHT_HELPERS`가 가리키는 helper를 먼저 import해서 재사용한다.
  - `getByTestId`를 사용하기 전에는 반드시 페이지에서 실제 `data-testid`를 수집하고, 기대한 test id가 존재하는지 확인한 뒤에만 locator를 작성한다.
  ...
  - 재실행 기본 명령은 `cargo run --bin rc -- run-playwright-qa --web-root /home/tree/project/mono_Manager/assets/web -- <same command>` 를 사용한다.
...
- UI 작업이면 `job.md#check` 아래에 `logic_checklist`, `ui_checklist`를 유지한다. 상태 변화 작업이면 `persistence_checklist`, `reentry_checklist`, `negative_checklist`도 유지한다.
- 이때 `item` 입력 형식은 `draft_item.name`:`검사해볼 기능` 형식으로 작성한다.
- 검증 체크리스트에 알맞은 유닛테스트들을 생성한다. 
- 사용자 입력들이 필요한 경우 mock 객체를 생성해서 사용 
- 유넷 테스트 실행후 pass 확인 
- 실행 결과를 `job.md#check evidence`에 `- [x| ] 입력 -> 출력 : 기능설명` 형식으로 남긴다.
...
- UI 항목은 `execution=browser`, `data source`, `artifact`를 함께 남긴다. 브라우저 실패 뒤 API/fixture 우회만 남으면 완료 처리하면 안 된다.
...
- QA 재실행 시 `ORC_QA_PLAYWRIGHT_HELPERS` 절대경로를 함께 전달해 ad-hoc 스크립트가 공용 helper를 import할 수 있게 유지한다.
...
- UI/웹 증상 해소 판정은 `execution=browser`가 확인된 실제 브라우저 검증으로만 내린다. `real-equivalent`는 persistence/re-entry 보조 근거로만 사용한다.
