...
- 유닛 테스트는 보조 검증일 뿐 완료 판정이 아니다.
- draft의 `check`를 충족하는 실제 실행 근거를 만들 수 있는 방향으로 구현한다.
- 상태 변화 기능이면 reload/reopen 뒤 selector/assert 또는 동등한 재진입 검증이 가능해야 한다.
- `step`의 순서대로 구현한다.
...
- `유닛테스트 pass`, `로컬 빌드 성공`만으로 완료라고 쓰지 않는다.
- 실제 완료 판정은 `check_orc_code`와 `job.md#check evidence` 실행 증거가 같이 있을 때만 가능하다.
